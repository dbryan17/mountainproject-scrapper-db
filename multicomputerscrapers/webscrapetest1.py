...
# the files in this are set up to scrape the rest of mtp
    ...
    for cotent in descriptions:
        ...
        if (not isinstance(td, int)):
            ...
            if ("Page Views" in td.string):
                ...
                str_arr = nextNextSibling.string.split(",")
                pitches = 1
                length = None
                types = []
                commitment_rating = 'I'
...
def getStats(route, url):
    ...
    for content in contents:
        if ("Star Ratings" in content.string):
            #starsAndPeople = []
            # CAN ALSO GET ACTUAL AVERAGE FROM HERE, BUT MIGHT BE BEST TO STICK TO PUBLIC USERS
            # total_with_private = content.string.split("Ratings", 1)[1].strip()
        ...
        elif ("On To-Do Lists" in content.string):
            ...
            for todoer_outer in todoers_outer:
                ...
                insert_todo(dbTodo)
        elif ("Suggested Ratings" in content.string):
            # this matches below - always?????
            # rated_with_private = content.string.split("Ratings", 1)[1].strip()
            # gradesAndPeople = []
        ...
        elif ("Ticks" in content.string):
            #ticks = []
            # total_ticks_with_private = content.string.split("Ticks", 1)[
            # 1].strip()
    ...
    for poss in privates_outer_poss:
        if (poss.string == "No names/notes"):
            private_outer = poss.parent.next_sibling.next_sibling.find_all(
                "strong")
            for private in private_outer:

                if (private.string.strip() == "-no date-"):
                    date = None
                else:
                    date = private.string

                dbTick = ((route, date))
                #ticks.append((None, private.string, None, None, None, None))
...
def getPeople(url):
    ...
    display_name = soup.title.string.split(" on ", 1)[0]
    begin = soup.find("div", class_="info mt-1")
    start_date = begin.contents[3].string
    points = begin.find(class_="mt-1").contents[1].string.split(" ", 1)[0]
    ...
    for pi in pi_contents:
        if ("·" not in pi.string and i == 0):
            if (pi.string.strip() != ""):
                if ("years old" in pi.string.strip()):
                    age = pi.string.strip().split("years old", 1)[0]
                    break
        if ("·" in pi.string):
            split_arr = pi.string.strip().split("·")
    ...
    for bio in bios:
        ...
        if (strong):
            if ("Other Interests" == strong.string):
                other_interests_string = bio.contents[3].string
...
if __name__ == "__main__":
    ...
    outer_areas = [#"https://www.mountainproject.com/area/105905173/alabama",
...
#     pages = int(pages_list[len(pages_list) - 1].string.split("of", 1)[1].strip())
...
#                 "div", class_="float-xs-right string-warm small mt-half").string.strip()
#             comment_string = comment_row.find(
#                 "span", class_="new-indicator").next_sibling.strip()
#             if ("/route/" in link):
#                 # comment on a route
#                 route_comments.append((link, comment_date, comment_string))

#             elif ("/area/" in link):
#                 # comment on an area
#                 area_comments.append((link, comment_date, comment_string))
